# routes/prompts/folders/reorder_folders.py
...
async def reorder_folders(
    ...
    """Reorder folders within a parent folder or at root level."""
    try:
        ...
        for item in request.items:
            ...
            if not verify_response.data:
                raise HTTPException(status_code=404, detail=f"Folder {item.id} not found or access denied")
            ...
            supabase.table("prompt_folders").update(update_data) \
                .eq("id", item.id) \
                .eq("user_id", user_id) \
                .execute()
        ...
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=500, detail=f"Error reordering folders: {str(e)}")